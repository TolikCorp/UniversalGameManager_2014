...
# Физическое расположение сервера
    server_dir="/home/usert/csgo_1"
# Название окна с сервером    
    server_screen_title="csgo_1"
# APP ID Dedicated Server
    server_app_id="740"
# Название игры для srcds_run
    server_run_title="csgo"
...
####################################################
    while true; do
        cd $server_dir
        ./srcds_run -console -usercon -game ${game_name_run} -tickrate ${tickrate} -secure +game_type 0 +game_mode 0 +mapgroup mg_bomb +ip ${ip_address} +map ${default_map} -port ${game_port} +clientport ${client_port} +tv_port ${tv_port} -steamport ${steam_port} -maxplayers_override ${maxplayers} ${params_ext} $autoupdate
